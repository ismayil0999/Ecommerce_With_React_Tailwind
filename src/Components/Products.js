        ...
        {products.slice(0,visible).map(product=>{
          return (
            <div key={product.id} className='rounded-2xl flex flex-col w-11/12  relative pt-8 h-300 rounded  sm:w-full md:w-1/1 lg:w-1/1 gap-8 shadow-sm bg-white hover:translate-y-1 duration-200 transition-all'>
              <FavoriteBorderIcon onClick={(e)=>{dispatch(addfavorite({id:product.id,tag:e.target, element:e.currentTarget}))}} className={item.includes(product) ? "text-red-600 absolute right-2 top-2 font-bold" : 'absolute right-2 top-2 font-bold text-black'}/>
              ...
              <button className='bg-blue-600 text-white p-2 font-bold mt-auto'  onClick={()=>{dispatch(addtocart({id:product.id}))}}  >Add to Cart</button>
            </div>
          )
        })}
      </div>
