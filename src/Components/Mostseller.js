...
export default function Mostseller(){
    ...
    useEffect(()=>{
        const interval = setInterval(() => {
            if (div.current) {
              div.current.scrollTop += 100; // Kaydırma miktarını burada ayarlayabilirsiniz
            }
          }, 1000); // Kaydırma hızını burada ayarlayabilirsiniz
    ...
    return (
      <div ref={div} className="flex bg-[#1976D2] pb-[50px] pt-[50px]">
        ...
        <div className="flex flex-col items-center justify-start rotate-[-10deg]">
         ...
         <img src="./headphone2.png" className="w-[300px] h-[300px]"></img>
            </div>
        </div>
      </div>
    )
