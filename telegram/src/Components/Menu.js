...
<ul className='w-full'>
<h1 className='text-2xl pl-2 text-disc font-bold bg-black text-white p-2'>Categories</h1>
  {  
      items.map((item)=>{
        return <li key={item.key} className=' pt-2 pb-2 pl-2 flex gap-4 font-bold hover:text-red-600' onClick={(e)=>{dispatch(filter({category:e.currentTarget}))}}><span className={item.class}>{item.icon}</span>{item.title}</li>
      })
  }
  </ul>
<ul className='w-full'>
<h1 className='text-2xl pl-2 text-disc font-bold bg-black text-white p-2'>Filter products</h1>
  {
    filteritem.map((item)=>{
        return <li key={item.key} className=' pt-2 pb-2 pl-2 flex gap-4 font-bold hover:text-red-600'><span className={item.class}>{item.icon}</span>{item.title}</li>
      })
  }
  </ul>
    </div>
