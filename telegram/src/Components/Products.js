...
import { getContacts,addfavorite } from '../redux/data';
...
export default function Products() {
  ...
  return (
    <div className='flex flex-col z-50 w-full  lg:pt-8  bg-body pb-16 items-center'>
      ...
      <ArrowForwardIcon/>
      <p className='text-xl font-bold text-red-600'> {products.length===0 ? "no item" : products[0].category}</p>
    </div>
        ...
        {products.slice(0,visible).map(product=>{
          return (
            <div key={product.id} className='rounded-2xl flex flex-col w-11/12  relative pt-8 h-300 rounded  sm:w-full md:w-1/1 lg:w-1/1 gap-8 shadow-sm bg-white hover:translate-y-1 duration-200 transition-all'>
              ...
              <button className='bg-red-600 text-white p-2 font-bold mt-auto'>Add to Cart</button>
            </div>
          )
        })}
      </div>
      {products.length<visible ? <button className='bg-grey-400 w-full sm:w-52 p-2 rounded-2xl text-white font-bold mt-8' onClick={()=>{setVisible((prevValue)=>prevValue+4)}}>No More....</button> : <button className='bg-red-600 w-full sm:w-52 p-2 rounded-2xl text-white font-bold mt-8' onClick={()=>{setVisible((prevValue)=>prevValue+4)}}>Load More....</button>}
    </div>
  </div>
  );
}
